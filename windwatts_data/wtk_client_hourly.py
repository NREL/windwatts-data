...
class WTKLedClientHourly(client_base):
    ...
    def download_hourly_data(self,
        ...
        if years is None:
            raise ValueError("At least 1 value for the list parameter 'years' must be specified.")
    ...
    def compute_statistic(self,
        ...
        if columns is not None:
            if not isinstance(columns, list) or not all(isinstance(col, str) for col in columns):
                raise TypeError("Parameter 'columns' must be a list of valid column names (strings).")

        ## Validate `years`
        if years is not None:
            if not isinstance(years, list) or not all(isinstance(year, int) for year in years):
                raise TypeError("Parameter 'years' must be a list of integers representing years.")
        
        ## Validate `months`
        if months is not None:
            if not isinstance(months, list) or not all(isinstance(month, int) and 1 <= month <= 12 for month in months):
                raise TypeError("Parameter 'months' must be a list of integers representing months.")
         # Validate `days`
        if days is not None:
            if not isinstance(days, list) or not all(isinstance(day, int) and 1 <= day <= 31 for day in days):
                raise TypeError("Parameter 'days' must be a list of integers (1-31).")
            
        ## Validate `hours`
        if hours is not None:
            if not isinstance(hours, list) or not all(isinstance(hour, int) and 0 <= hour <= 23 for hour in hours):
                raise TypeError("Parameter 'hours' must be a list of integers representing hours[0-23].")
        
        ## Validate `heights`
        if heights is not None:
            if not isinstance(heights, list) or not all(isinstance(height, (int, float)) for height in heights):
                raise TypeError("Parameter 'heights' must be a list of numeric values (int or float).")
            if columns is not None:
    ...
    def compute_average_windspeed(self,
        lat: float = None,
        long: float = None,
        height: float = None,
        group_by_year: bool = False,
    ...
    def fetch_windspeed_map(self, 
        ...
        varset: str = "all") -> pd.DataFrame:
        """
        Fetch windspeed map for a given height and filter by speific year, month, day and hour.

        :param height: Height for which wind data should be retrieved. This parameter is required.
        :type height: float
        :param year: Year to filter the data. This parameter is required.
        :type year: int
        :param month: Month to filter the data. This parameter is required. Range(1-12).
        :type month: int
        :param day: Day to filter the data. This parameter is required. Range(1-31).
        :type day: int
        :param hour: Hour to filter the data. This parameter is required. Range(0-23).
        :type hour: int
        :param varset: Variable set to filter the data. Default is 'all'.(Takes only default value for now)
        :type varset: str
        ...
        :return: A pandas DataFrame with windspeed map.
        """
        # Validate heights
        if height is None:
        ...
        if year is None or month is None or hour is None or day is None:
            raise ValueError("Values for parameters 'year', 'month', 'day' and 'hour' list must be specified.")
        ...
        if hour is not None:
            if not isinstance(hour, int) or not 1<=hour<=24:
                raise ValueError("Parameter 'hour' must be a integer with range (0-23).")
        
        # Validate `day`
        if day is not None:
            if not isinstance(day, int) or not 1<=day<=31:
                raise ValueError("Parameter 'day' must be a integer with range (1-31).")
        ...
        if month:
            query += f" AND SUBSTRING(CAST(time_index AS VARCHAR), 5, 2) = '{month:02}'"

        if day:
            query += f" AND SUBSTRING(CAST(time_index AS VARCHAR), 7, 2) = '{day:02}'"

        if hour:
            query += f" AND SUBSTRING(CAST(time_index AS VARCHAR), 9, 2) = '{hour:02}'"
    ...
    def fetch_winddirection_map(self, 
        height: float = None, 
        year: int = None,
        month: int = None,
        day: int = None,
        ...
        varset: str = "all") -> pd.DataFrame:
        """
        Fetch winddirection map for a given height and filter by speific year, month, day and hour.
    ...
    def fetch_filtered_data(self,
        ...
        if columns is not None:
            if not isinstance(columns, list) or not all(isinstance(col, str) for col in columns):
                raise TypeError("Parameter 'columns' must be a list of strings representing column names.")
